...
if [ -n "${TEST_DATABASE_URL:-}" ]; then
  ...
  $RUN "PATH=\$PATH:/usr/lib/postgresql/16/bin pg_ctl -D $PGROOT/data -o '-p 55433 -k $PGROOT' -l $PGROOT/log start" >/dev/null

  until psql -h "$PGROOT" -p 55433 -U postgres -c 'select 1' >/dev/null 2>&1; do sleep 0.3; done
  psql -h "$PGROOT" -p 55433 -U postgres -qc 'create database h365test;'

  export TEST_DATABASE_URL="postgresql://postgres@localhost:55433/h365test?host=$PGROOT"
  PSQL_ARGS=(-h "$PGROOT" -p 55433 -U postgres -d h365test -q -v ON_ERROR_STOP=1)
